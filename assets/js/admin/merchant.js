...
var refreshTableData = function(){
    ...
    loadMerchant(localData.merchant.searchFromHolder.id, function(error, data){
        if(!error){
    ...
    $("#select-merchant-type").select2({
        ajax: {
            ...
            data: function (params) {
                return {
                    and: '{"org._id":"'+localData.merchant.searchFromHolder+'","name":\'\'*'+params.term+'*\'\'}', // search term
                    page: params.page
                };
            },
...
populate.merchant = function(){
	...
	if(typeof localData.merchant.searchFromHolder !== 'undefined'){
        // manually add data to the Select2 component and select it
        var newOption = new Option(localData.merchant.searchFromHolder.name, localData.merchant.searchFromHolder.id, true, true);
        $("#select-holder").append(newOption).trigger('change');    
    }
