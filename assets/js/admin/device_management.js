...
var setNewDeviceBtn = function(_deviceObj){
    ...
    if(typeof _deviceObj === "undefined"){
        ...
        $('#lp-lat').val('');
        $('#lp-lon').val('');
        // check existing select2 element and deselect
        if ($('#select-holder-device').hasClass("select2-hidden-accessible")) {
            $("#select-holder-device").select2("val", ""); 
        }
        if ($('#select-merchant-device').hasClass("select2-hidden-accessible")) {
            $("#select-merchant-device").select2("val", ""); 
        }
        
        $("#select-holder-device").removeAttr('disabled');
        $("#select-merchant-device").removeAttr('disabled');
        $("#select-holder-device").html('');
        $("#select-merchant-device").html('');
        ...
        $("#select-holder-device").append(newOption).trigger('change');
        $("#select-merchant-device").append(newOption).trigger('change');
        $("#select-holder-device").attr('disabled', 'disabled');
        $("#select-merchant-device").attr('disabled', 'disabled');
    }
...
var configTableDevice = function(){
    ...
    for(i=0; i<localData.device.data.length; i++){
        ...
        table.DataTable().row.add([localData.device.data[i].name, localData.device.data[i].org.name, localData.device.data[i].property.number, (typeof localData.device.data[i].property.vendor === 'undefined'? '':localData.device.data[i].property.vendor), (typeof localData.device.data[i].property.series === 'undefined'? '':localData.device.data[i].property.series), additional_el]);
    }
...
var configSelectHolderDevice = function(){
    $("#select-holder-device").select2({
        ajax: {
    ...
    $('#select-holder-device').on('change', function(){
        if($('#select-holder-device').val() !== null && $('#select-holder-device').val() !== 'null'){
            localData.selectedOrg = {
                id: $("#select-holder-device").val(),
                name: $("#select-holder-device").select2('data')[0].name || $("#select-holder-device").select2('data')[0].text
            }
            configSelectMerchantDevice($("#select-holder-device").val());
        } else {
...
var configSelectMerchantDevice = function(_holderId){
    $("#select-merchant-device").select2({
        ajax: {
            url: "http://192.168.100.50:8000/rv1/org",
            dataType: 'json',
            delay: 500,
            ...
            data: function (params) {
                return {
                    and: '{"org._id":"'+ _holderId +'","name":\'\'*'+params.term+'*\'\'}', // search term
                    page: params.page
                };
            },
            processResults: function (data, params) {
    ...
    $('#select-merchant-device').on('change', function(){
        if($('#select-merchant-device').val() !== null){
            localData.selectedMerchant = {
                id: $("#select-merchant-device").val(),
                name: $("#select-merchant-device").select2('data')[0].name || $("#select-merchant-device").select2('data')[0].text
            }
        } else {
...
init.device_management = function(cb){
    loadDevice(function(error, data){
        if(!error){
            ...
            cb({success: false, message: "Error fetching device data from server"});
        }
    });
}
...
populate.device_management = function(){
    ...
    $('#wizard a[href="#finish"]').on('click', function(e){
        ...
        if($("#select-merchant-device").val() !== null && $("#select-merchant-device").val() !== 'null'){
            deviceData.org = {
                _id: $('#select-merchant-device').val()
            }
        }
