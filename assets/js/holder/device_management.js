...
var setNewDeviceBtn = function(_deviceObj){
    ...
    if(typeof _deviceObj === "undefined"){
        ...
        var newOption = new Option(_deviceObj.org.name, _deviceObj.org._id, true, true);
...
var loadMerchant = function(_orgId, cb){
    cb(false, 1);
    // placeholder function, in case the module will be updated so that it 
    // can load data from multiple merchant from a holder
}
...
var refreshDeviceTableData = function(){
    ...
    loadMerchant(getCookie('orgid'), function(error, data){
        if(!error){
            loadDevice($('#select-merchant-device').val(), function(error, data){
                if(!error){
                    configTableDevice();
                }
...
var loadDevice = function(_orgId, cb){
    var params = '';
    try {
        var searchkey = typeof localData.device.searchparam === 'undefined'?'':localData.device.searchparam;
        params = '&and={%22org._id%22:"'+_orgId+'", %22name%22:%27%27*'+searchkey+'*%27%27}&page='+localData.device.property.paging.page+'&limit='+localData.device.property.paging.limit;
    } catch(err) {
...
populate.device_management = function(){
    ...
    configSelectMerchantDevice(getCookie('orgid'));
